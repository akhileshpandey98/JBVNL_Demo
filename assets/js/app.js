...
function filterNotice(type, btn) {
  ...
  document
    ...
    .forEach(
      (x) =>
        (x.style.display =
          type === "all" || x.dataset.type === type ? "flex" : "none"),
    );
}

function fontSize(n) {
  document.body.style.fontSize = n === 1 ? "17px" : n === -1 ? "13px" : "15px";
}
function toggleHindi() {
  alert("Hindi interface can be connected to the JBVNL Hindi content/routes.");
}
function searchSite() {
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function closeQuickLinks() {
    ...
    quickLinksBtn.style.animation = "quickLinksBlink 1.4s infinite";
  }
